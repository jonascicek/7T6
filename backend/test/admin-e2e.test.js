...
test('admin UI creates a post and it becomes visible on the homepage', async (t) => {
  ...
  const frontendProcess = spawn('npm', ['run', 'dev', '--', '--host', '127.0.0.1', '--port', String(frontendPort)], {
  ...
  t.after(async () => {
    await prisma.post.deleteMany({ where: { title: { contains: 'Admin E2E' } } })
  ...
  try {
    ...
    await page.goto(`${frontendUrl}/admin`, { waitUntil: 'domcontentloaded' })
    ...
    assert.equal(loginUrl, `${frontendUrl}/admin/login`)

    await page.goto(`${frontendUrl}/admin`, { waitUntil: 'domcontentloaded' })
    await page.waitForTimeout(2000)
    const adminUrl = page.url()
    assert.ok(adminUrl.startsWith(`${frontendUrl}/admin`))

    await page.goto(`${frontendUrl}/`, { waitUntil: 'domcontentloaded' })
    await page.waitForTimeout(3000)
    const homepageUrl = page.url()
    const homepageText = await page.locator('body').innerText()
    assert.equal(homepageUrl, `${frontendUrl}/`)
